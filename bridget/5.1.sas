...
DATA lines;
	...
	*Finding totals per x,y position;
	DO i = 0 to moves;
		...
		if ((x1 ne x2) AND (y1 ne y2)) then danger(position_y,position_x) = danger(position_y,position_x); *horizontal or vertical lines only;
		else if danger(position_y,position_x) = . then danger(position_y,position_x) = 1;
