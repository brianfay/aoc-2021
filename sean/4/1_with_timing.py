...
sc = SparkContext(conf=SparkConf().setAppName('aoc-2021-4.1t').setMaster('local'))
broadcast_numbers = sc.broadcast(drawn_numbers)
time_log("Initialized spark")
rdd = sc.parallelize(data).cache()
time_log("Parallelized data")
